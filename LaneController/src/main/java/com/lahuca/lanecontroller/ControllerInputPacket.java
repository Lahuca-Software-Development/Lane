        ...
        switch (iPacket) {
            case GameStatusUpdatePacket(long requestId, GameRecord record) -> {
                ...
                if (!input.from().equals(record.instanceId())) {
                    getConnection().sendPacket(new VoidResultPacket(requestId, ResponseError.INSUFFICIENT_RIGHTS), input.from());
                    return;
                }
                if (!games.containsKey(gameId)) {
                    // A new game has been created, yeey!
                    games.put(gameId, new ControllerGame(record));
                    getConnection().sendPacket(new VoidResultPacket(requestId), input.from());
                    return;
                }
            ...
            case GameShutdownPacket(long requestId, long gameId) -> {
                ControllerGame game = games.get(gameId);
                if (game == null) {
                    getConnection().sendPacket(new VoidResultPacket(requestId, ResponseError.INVALID_ID), input.from());
                    return;
                }
            ...
            case PartyPacket.Retrieve.Request packet ->
                    getPartyManager().getParty(packet.partyId()).ifPresentOrElse(
                            party -> getConnection().sendPacket(new PartyPacket.Retrieve.Response(packet.getRequestId(), null, party.convertRecord()), input.from()),
                            () -> getConnection().sendPacket(new PartyPacket.Retrieve.Response(packet.getRequestId(), ResponseError.INVALID_ID), input.from()));
            case PartyPacket.Retrieve.RequestPlayerParty packet ->
                    getPlayer(packet.player()).ifPresentOrElse(player -> player.getParty().ifPresentOrElse(party -> getConnection().sendPacket(new PartyPacket.Retrieve.Response(packet.getRequestId(), null, party.convertRecord()), input.from()), () -> {
                        if (!packet.createIfNeeded()) {
                            getConnection().sendPacket(new PartyPacket.Retrieve.Response(packet.getRequestId(), ResponseError.ILLEGAL_ARGUMENT), input.from());
                            return;
                        }
                        getPartyManager().createParty(player).ifPresentOrElse(newParty -> getConnection().sendPacket(new PartyPacket.Retrieve.Response(packet.getRequestId(), null, newParty.convertRecord()), input.from()), () -> getConnection().sendPacket(new PartyPacket.Retrieve.Response(packet.getRequestId(), ResponseError.ILLEGAL_ARGUMENT), input.from()));
                    }), () -> getConnection().sendPacket(new PartyPacket.Retrieve.Response(packet.getRequestId(), ResponseError.INVALID_ID), input.from()));
            ...
            case PartyPacket.Operations.AcceptInvitation packet ->
                    getPartyManager().getParty(packet.partyId()).ifPresentOrElse(party ->
                                    getPlayerManager().getPlayer(packet.player()).ifPresentOrElse(player ->
                                                    getConnection().sendPacket(new SimpleResultPacket<>(packet.getRequestId(), null, party.acceptInvitation(player)), input.from()),
                                            () -> getConnection().sendPacket(new SimpleResultPacket<Boolean>(packet.getRequestId(), ResponseError.INVALID_PLAYER), input.from())),
                            () -> getConnection().sendPacket(new SimpleResultPacket<Boolean>(packet.getRequestId(), ResponseError.INVALID_ID), input.from()));
            case PartyPacket.Operations.AddInvitation packet ->
                    getPartyManager().getParty(packet.partyId()).ifPresentOrElse(party ->
                                    getPlayerManager().getPlayer(packet.player()).ifPresentOrElse(player ->
                                                    getConnection().sendPacket(new SimpleResultPacket<>(packet.getRequestId(), null, party.addInvitation(player)), input.from()),
                                            () -> getConnection().sendPacket(new SimpleResultPacket<Boolean>(packet.getRequestId(), ResponseError.INVALID_PLAYER), input.from())),
                            () -> getConnection().sendPacket(new SimpleResultPacket<Boolean>(packet.getRequestId(), ResponseError.INVALID_ID), input.from()));
            case PartyPacket.Operations.Create packet ->
                    getPlayerManager().getPlayer(packet.player()).ifPresentOrElse(player -> {
                        if (player.getParty().isPresent()) {
                            getConnection().sendPacket(new SimpleResultPacket<Boolean>(packet.requestId(), ResponseError.ILLEGAL_STATE), input.from());
                            return;
                        }
                        getPartyManager().createParty(player).ifPresentOrElse(party ->
                                        getConnection().sendPacket(new SimpleResultPacket<>(packet.requestId(), party.convertRecord()), input.from()),
                                () -> getConnection().sendPacket(new SimpleResultPacket<>(packet.requestId(), ResponseError.ILLEGAL_STATE), input.from()));
                    }, () -> getConnection().sendPacket(new SimpleResultPacket<Boolean>(packet.getRequestId(), ResponseError.INVALID_PLAYER), input.from()));
            case PartyPacket.Operations.DenyInvitation packet ->
                    getPartyManager().getParty(packet.partyId()).ifPresentOrElse(party ->
                                    getPlayerManager().getPlayer(packet.player()).ifPresentOrElse(player ->
                                                    getConnection().sendPacket(new SimpleResultPacket<>(packet.getRequestId(), party.denyInvitation(player)), input.from()),
                                            () -> getConnection().sendPacket(new SimpleResultPacket<Boolean>(packet.getRequestId(), ResponseError.INVALID_PLAYER), input.from())),
                            () -> getConnection().sendPacket(new SimpleResultPacket<Boolean>(packet.getRequestId(), ResponseError.INVALID_ID), input.from()));
            ...
            case PartyPacket.Operations.JoinPlayer packet ->
                    getPartyManager().getParty(packet.partyId()).ifPresentOrElse(party ->
                                    getPlayerManager().getPlayer(packet.player()).ifPresentOrElse(player ->
                                                    getConnection().sendPacket(new SimpleResultPacket<>(packet.getRequestId(), party.joinPlayer(player)), input.from()),
                                            () -> getConnection().sendPacket(new SimpleResultPacket<Boolean>(packet.getRequestId(), ResponseError.INVALID_PLAYER), input.from())),
                            () -> getConnection().sendPacket(new SimpleResultPacket<Boolean>(packet.getRequestId(), ResponseError.INVALID_ID), input.from()));
            case PartyPacket.Operations.RemovePlayer packet ->
                    getPartyManager().getParty(packet.partyId()).ifPresentOrElse(party ->
                                    getPlayerManager().getPlayer(packet.player()).ifPresentOrElse(player ->
                                                    getConnection().sendPacket(new SimpleResultPacket<>(packet.getRequestId(), party.removePlayer(player)), input.from()),
                                            () -> getConnection().sendPacket(new SimpleResultPacket<Boolean>(packet.getRequestId(), ResponseError.INVALID_PLAYER), input.from())),
                            () -> getConnection().sendPacket(new SimpleResultPacket<Boolean>(packet.getRequestId(), ResponseError.INVALID_ID), input.from()));
            ...
            case PartyPacket.Operations.SetOwner packet ->
                    getPartyManager().getParty(packet.partyId()).ifPresentOrElse(party ->
                                    getPlayerManager().getPlayer(packet.player()).ifPresentOrElse(player ->
                                                    getConnection().sendPacket(new SimpleResultPacket<>(packet.getRequestId(), party.setOwner(player)), input.from()),
                                            () -> getConnection().sendPacket(new SimpleResultPacket<Boolean>(packet.getRequestId(), ResponseError.INVALID_PLAYER), input.from())),
                            () -> getConnection().sendPacket(new SimpleResultPacket<Boolean>(packet.getRequestId(), ResponseError.INVALID_ID), input.from()));
            ...
            case QueueRequestPacket packet -> {
                if (packet.parameters() == null) {
                    getConnection().sendPacket(new VoidResultPacket(packet.getRequestId(), ResponseError.ILLEGAL_ARGUMENT), input.from());
                    return;
                }
                getPlayer(packet.player()).ifPresentOrElse(player -> player.queue(new QueueRequest(QueueRequestReason.PLUGIN_INSTANCE, packet.parameters()), true).whenComplete((result, exception) -> {
                    if (exception != null) getConnection().sendPacket(new VoidResultPacket(packet.getRequestId(), new ResponseError(exception)), input.from());
                    getConnection().sendPacket(new VoidResultPacket(packet.getRequestId()), input.from());
                }), () -> getConnection().sendPacket(new VoidResultPacket(packet.getRequestId(), ResponseError.INVALID_PLAYER), input.from()));
            }
                    ...
                    player.getQueueRequest().ifPresentOrElse(queue -> {
                        ...
                        if (state != null && state.getProperties() != null && state.getProperties().containsKey(LaneStateProperty.INSTANCE_ID)) {
                            ...
                            if (state.getName().equals(LanePlayerState.INSTANCE_TRANSFER) && state.getProperties().get(LaneStateProperty.INSTANCE_ID).getValue().equals(input.from())) {
                                ...
                                getConnection().sendPacket(new VoidResultPacket(packet.getRequestId(), ResponseError.ILLEGAL_STATE), input.from());
                            }
                            return;
                        }
                        getConnection().sendPacket(new VoidResultPacket(packet.getRequestId(), ResponseError.ILLEGAL_STATE), input.from());
            ...
            case RequestIdPacket packet -> {
                ...
                if (newId == null) {
                    getConnection().sendPacket(new LongResultPacket(packet.getRequestId(), ResponseError.ILLEGAL_ARGUMENT), input.from());
                } else {
                    // TODO Do reservation, we do not want doubles!
                    getConnection().sendPacket(new LongResultPacket(packet.getRequestId(), newId), input.from());
                }
            }
            case DataObjectReadPacket packet -> {
                if (!packet.permissionKey().isIndividual()) {
                    getConnection().sendPacket(new DataObjectResultPacket(packet.getRequestId(), ResponseError.ILLEGAL_ARGUMENT), input.from());
                }
                dataManager.readDataObject(packet.permissionKey(), packet.id()).whenComplete((object, ex) -> {
                    if (ex != null) {
                        getConnection().sendPacket(new DataObjectResultPacket(packet.getRequestId(), new ResponseError(ex)), input.from());
                    } else {
                        getConnection().sendPacket(new DataObjectResultPacket(packet.getRequestId(), object.orElse(null)), input.from());
                    }
                });
            }
            case DataObjectWritePacket packet -> {
                ...
                dataManager.writeDataObject(packet.permissionKey(), packet.object()).whenComplete((bool, ex) -> { // TODO This template with Void can be put in function!
                    if (ex != null) {
                        getConnection().sendPacket(new VoidResultPacket(packet.getRequestId(), new ResponseError(ex)), input.from());
                    } else {
                        getConnection().sendPacket(new VoidResultPacket(packet.getRequestId()), input.from());
                    }
                });
            ...
            case DataObjectRemovePacket packet -> {
                if (!packet.permissionKey().isIndividual()) {
                    getConnection().sendPacket(new SimpleResultPacket<>(packet.getRequestId(), ResponseError.ILLEGAL_ARGUMENT), input.from());
                }
            ...
            case DataObjectListIdsPacket packet -> {
                dataManager.listDataObjectIds(packet.prefix()).whenComplete((object, ex) -> {
                    if (ex != null) {
                        getConnection().sendPacket(new DataObjectIdsResultPacket(packet.getRequestId(), new ResponseError(ex)), input.from());
                    } else {
                        getConnection().sendPacket(new DataObjectIdsResultPacket(packet.getRequestId(), object), input.from());
                    }
                });
            }
            ...
            case RequestInformationPacket.Players packet -> {
                ...
                getConnection().sendPacket(new RequestInformationPacket.PlayersResponse(packet.getRequestId(), null, data), input.from());
            }
            case RequestInformationPacket.Game packet ->
                    getConnection().sendPacket(new RequestInformationPacket.GameResponse(packet.getRequestId(), null, getGame(packet.gameId()).map(ControllerGame::convertRecord).orElse(null)), input.from());
            case RequestInformationPacket.Games packet -> {
                ...
                getConnection().sendPacket(new RequestInformationPacket.GamesResponse(packet.getRequestId(), null, data), input.from());
            }
            case RequestInformationPacket.Instance packet -> {
                getConnection().sendPacket(new RequestInformationPacket.InstanceResponse(packet.getRequestId(), null, getInstance(packet.id()).map(ControllerLaneInstance::convertRecord).orElse(null)), input.from());
            }
            case RequestInformationPacket.Instances packet -> {
                ...
                getConnection().sendPacket(new RequestInformationPacket.InstancesResponse(packet.getRequestId(), null, data), input.from());
            }
            case RequestInformationPacket.PlayerUsername packet ->
                    getPlayerManager().getPlayerUsername(packet.uuid()).whenComplete((username, ex) -> {
                        if (ex != null) {
                            getConnection().sendPacket(new SimpleResultPacket<>(packet.getRequestId(), new ResponseError(ex)), input.from());
                            return;
                        }
                        getConnection().sendPacket(new SimpleResultPacket<>(packet.getRequestId(), username.orElse(null)), input.from());
                    });
            case RequestInformationPacket.PlayerUuid packet ->
                    getPlayerManager().getPlayerUuid(packet.username()).whenComplete((uuid, ex) -> {
                        ...
                        getConnection().sendPacket(new SimpleResultPacket<>(packet.getRequestId(), uuid.orElse(null)), input.from());
                    });
            case RequestInformationPacket.PlayerNetworkProfile packet ->
                    getPlayerManager().getPlayerNetworkProfile(packet.uuid())
                            .whenComplete((data, ex) -> {
                                if (ex != null) {
                                    getConnection().sendPacket(new ProfileRecordResultPacket(packet.getRequestId(), new ResponseError(ex)), input.from());
                                    return;
                                }
                                getConnection().sendPacket(new ProfileRecordResultPacket(packet.getRequestId(), data.map(ProfileData::convertRecord).orElse(null)), input.from());
                            });
            ...
            case SavedLocalePacket.Get packet -> {
                getDataManager().getProfileData(packet.networkProfile()).thenApply(opt -> opt.orElse(null))
                        ...
                        .whenComplete((locale, ex) -> {
                            ...
                            if (locale.isPresent()) {
                                getConnection().sendPacket(new SimpleResultPacket<>(packet.getRequestId(), locale.get().toLanguageTag()), input.from());
                            } else {
                                getConnection().sendPacket(new SimpleResultPacket<>(packet.getRequestId(), null), input.from());
                            }
                        });
            }
            ...
            case SetInformationPacket setInformation -> {
                switch (setInformation) {
                    case SetInformationPacket.PlayerSetNickname(long requestId, UUID uuid, String nickname) ->
                            getPlayer(uuid).ifPresentOrElse(player -> player.setNickname(nickname).whenComplete((result, ex) -> {
                                if (ex != null) {
                                    getConnection().sendPacket(new VoidResultPacket(requestId, new ResponseError(ex)), input.from());
                                    return;
                                }
                                getConnection().sendPacket(new VoidResultPacket(requestId), input.from());
                            }), () -> getConnection().sendPacket(new VoidResultPacket(requestId, ResponseError.INVALID_PLAYER), input.from()));
        ...
        switch (profilePacket) {
            case ProfilePacket.GetProfileData packet ->
                    getDataManager().getProfileData(packet.uuid()).whenComplete((opt, ex) -> {
                        ...
                        getConnection().sendPacket(new ProfileRecordResultPacket(packet.getRequestId(), opt.map(ControllerProfileData::convertRecord).orElse(null)), input.from());
                    });
            case ProfilePacket.CreateNew packet ->
                    getDataManager().createNewProfile(packet.type()).whenComplete((profile, ex) -> {
                        ...
                        getConnection().sendPacket(new ProfileRecordResultPacket(packet.getRequestId(), profile.convertRecord()), input.from());
                    });
            ...
            case ProfilePacket.AddSubProfile packet -> getDataManager().getProfileData(packet.current())
                    ...
                    .whenComplete((status, ex) -> {
                        ...
                        getConnection().sendPacket(new SimpleResultPacket<>(packet.getRequestId(), status), input.from());
                    });
        ...
        switch (friendshipPacket) {
            case FriendshipPacket.GetInvitations(
                ...
                getPlayer(uuid).ifPresentOrElse(player ->
                        ...
                        () -> getConnection().sendPacket(new FriendshipInvitationsPacket(requestId, ResponseError.INVALID_PLAYER), input.from()));
            }
            case FriendshipPacket.ContainsInvitation(long requestId, FriendshipInvitation invitation) ->
                    getConnection().sendPacket(new SimpleResultPacket<>(requestId, getFriendshipManager().containsInvitation(invitation)), input.from());
            ...
            case FriendshipPacket.AcceptInvitation(long requestId, FriendshipInvitation invitation) ->
                    getFriendshipManager().acceptInvitation(invitation).whenComplete((result, ex) -> {
                        if (ex != null) {
                            getConnection().sendPacket(new VoidResultPacket(requestId, new ResponseError(ex)), input.from());
                            return;
                        }
            ...
            case FriendshipPacket.GetFriendship(long requestId, long friendshipId) -> {
                getFriendshipManager().getFriendship(friendshipId).whenComplete((result, ex) -> {
                    if (ex != null) {
                        getConnection().sendPacket(new SimpleResultPacket<>(requestId, new ResponseError(ex)), input.from());
                        return;
                    }
                    getConnection().sendPacket(new SimpleResultPacket<>(requestId, result), input.from());
                });
            }
            case FriendshipPacket.GetFriendships(long requestId, UUID uuid) ->
                    getPlayer(uuid).ifPresentOrElse(player -> getFriendshipManager().getFriendships(player).whenComplete((result, ex) -> {
                        if (ex != null) {
                            getConnection().sendPacket(new RelationshipRecordsPacket(requestId, new ResponseError(ex)), input.from());
                            return;
                        }
                        getConnection().sendPacket(new RelationshipRecordsPacket(requestId, result), input.from());
                    }), () -> getConnection().sendPacket(new RelationshipRecordsPacket(requestId, ResponseError.INVALID_PLAYER), input.from()));
            ...
            case FriendshipPacket.RemoveFriendship(long requestId, long friendshipId) -> {
                getFriendshipManager().getFriendship(friendshipId).whenComplete((friendship, ex) -> {
                    ...
                    if (friendship == null) {
                        getConnection().sendPacket(new SimpleResultPacket<>(requestId, ResponseError.INVALID_ID), input.from());
                    ...
                    getFriendshipManager().removeFriendship(new RelationshipRecord(friendshipId, friendship.players())).whenComplete((result, exception) -> {
                        if (exception != null) {
                            getConnection().sendPacket(new SimpleResultPacket<>(requestId, new ResponseError(exception)), input.from());
                            return;
                        }
                        getConnection().sendPacket(new SimpleResultPacket<>(requestId, result), input.from());
                    });
